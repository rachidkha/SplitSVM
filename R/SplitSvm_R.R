...
cv.SplitSvm <- function(x, y, num_lambdas_sparsity = 100, num_lambdas_diversity = 100, alpha = 1, num_models = 10,
                       tolerance = 1e-8, max_iter = 1e5, num_folds = 5, num_threads = 1, delta = 1){
  require(BERN)
 ...
 fit <-bern(x.permutation, y.permutation)
  lambda_sparsity = sort(fit$lambda)
  output <- Main_Ensemble_EN(x.permutation, y.permutation, num_lambdas_sparsity, num_lambdas_diversity, alpha, num_models, 
                             tolerance, max_iter, num_folds, num_threads, delta,lambda_sparsity)
  fn_call <- match.call()
  output <- construct.cv.SplitSvm(output, fn_call, x, y)
  return(output)
}
